...
variable "mac_addr_script" {
	type = string
	description = "Mac addr script, will be the same each time as long as the hostname doesn't change."
	default = "mac_addr.sh"
}
...
variable "commoninit_iso" {
	...
	default = "debian-dhcp-vm-commoninit.iso"
}
variable "disk_size" {
	type = number
...
variable "hostname" {
	type = string
	default = "dhcp-vm.domain.tld"
}
variable "static_ip" {
	type = string
	description = "Static IP because Debian doesn't like my DHCP setup."
	default = "IPHERE"
}
